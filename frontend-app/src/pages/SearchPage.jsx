...
function SearchPage() {
  ...
  const { songs, playSong } = usePlayer(); // 假设歌曲在全局可用
  ...
  return (
    <div className={`search-page flex-1 p-6 rounded-lg shadow-md bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 transition-colors duration-300 ease-in-out`}>
      <h2 className={`text-3xl font-semibold mb-6 text-gray-800 dark:text-white transition-colors duration-300 ease-in-out`}>搜索歌曲</h2>
      <input 
        type="text"
      ...
      {searchResults.length === 0 && searchTerm.trim() !== '' && (
        <p className={`text-gray-500 dark:text-gray-400 transition-colors duration-300 ease-in-out`}>没有匹配的歌曲。</p>
      )}
